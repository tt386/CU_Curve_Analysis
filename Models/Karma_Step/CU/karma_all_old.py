...
#NPR is the number of points until a point is listed in the output: if equal to nmx, only get last one
#SP is the listed output names - we say BP0, which will never appear, so get none shown

hb1_period = load(run1('HB1'),IPS=2,ICP=["I",11],NMX=100000000,NPR=100000,DS=0.001,DSMAX=0.1,SP=['BP0'],EPSL=1e-7,EPSU=1e-7)#,NTST=200,NCOL=4,RL0=0.1,RL1=1000)
#Stop when the period is massive
u_period = run(hb1_period,UZSTOP={'PERIOD':1000000})
p=plot(u_period)


#Use the end point given by UZstop of this and continue in I,C
#Only care about c between 0 and 3
homoclinic = load(u_period("UZ1"),IPS=2,ICP=["I","c"])
hom = run(homoclinic,UZSTOP={'c':[0,10]})
hom = hom + run(homoclinic,DS='-',UZSTOP={'c':[0,10]})
#hom = run(homoclinic,UZSTOP={'c':[0.810,0.811],'I':[-0.963,-0.961]})
#hom = hom + run(homoclinic,DS='-',UZSTOP={'c':[0.810,0.811],'I':[-0.963,-0.961]})
p=plot(hom + u)
p.config(bifurcation_y='c')
